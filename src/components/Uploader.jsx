...
const Uploader = () => {
  ...
  const createTable = (originArr, targetArr) => {
    return(
      <div className='segment-list'>
        <div className="tmx-buttons d-flex align-items-center">
  ...
  const segmentExtractor = (tmx) => {
    ...
    let originLangStart = tmx.indexOf('xml:lang="') + 10;
    let originLangEnd = tmx.indexOf('"', originLangStart);
    let originLang = tmx.substring(originLangStart, originLangEnd);
    originColumn.push(originLang.toLowerCase());

    let targetLangStart = tmx.indexOf('xml:lang="', originLangEnd) + 10;
    let targetLangEnd = tmx.indexOf('"', targetLangStart);
    let targetLang = tmx.substring(targetLangStart, targetLangEnd);
  ...
  return(
    <div>        
      ...
      { showSegments ? createTable(originColumn, targetColumn) : file ? <div className='file-uploaded'><p>File <em>{fileName}</em> uploaded!</p><div className="tmx-buttons d-flex align-items-center"><div className='btn-area'><Button variant='secondary' onClick={resetUpload} >Upload another file</Button></div><div className='btn-area'><Button variant='primary' onClick={showSegmentsClick} >Show segments</Button></div><CSVButton arr={ [originColumn, targetColumn] } /><CleanCSVButton arr={ [originColumn, targetColumn] } /> </div></div>: "" }
    </div>
  )
